...
class X2JPlugin : Plugin<Project> {
    ...
    private fun registerToGenerateX2JFiles(project: Project, android: BaseExtension, variant: BaseVariant) {
        ...
        project.tasks.create("generate${yaVariantName}X2J").apply {
            variant.registerJavaGeneratingTask(this, getJavaOutputDir(project, variant))
            group = "build"
            doLast {
                GlobalScope.launch {
                    generateX2JFiles(project, android, variant)
                }
            }
        }
    }
    ...
    private suspend fun generateX2JFiles(project: Project, android: BaseExtension, variant: BaseVariant) {
        ...
        while (!rFile.exists() || rFile.length() == 0L) {
            delay(20)
        }
        delay(20)
        ...
        if (isAndroidLibrary) {
            ...
            r2File.deleteOnExit()
            r2File.writeText(rFile.readText().replace("class R {", "class R2 {"))
        }

        val x2jFile = File(outputRootDir, "dev/android/x2j/X2J.java")
        x2jFile.parentFile.mkdirs()
        x2jFile.deleteOnExit()
        x2jFile.writeText(X2J_CODE
